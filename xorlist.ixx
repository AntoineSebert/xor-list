...
/*
 ...
 - [x] template<class T, class Alloc, class U> typename list<T, Alloc>::size_type erase(list<T, Alloc>& c, const U& value);
 - [x] template<class T, class Alloc, class Pred> typename list<T, Alloc>::size_type erase_if(list<T, Alloc>& c, Pred pred);
*/
/*
 - [x] template<class InputIt, class Alloc = std::allocator<typename std::iterator_traits<InputIt>::value_type>> list(InputIt, InputIt, Alloc = Alloc()) -> list<typename std::iterator_traits<InputIt>::value_type, Alloc>;
*/
...
export template <typename T, class Allocator = std::allocator<T>> class xorlist {
	...
	class iterator {
	  ...
	  public:
		...
		[[nodiscard]] pointer operator->() const noexcept { return pointer_traits<pointer>::pointer_to(**this); }
		auto operator++() {
			ptr++;
			return *this;
		}
	...
	class const_iterator {
	  ...
	  public:
		...
		const_pointer operator->() const noexcept { return pointer_traits<pointer>::pointer_to(**this); }
		auto operator++() {
			auto tmp = *this;
			++(*this);
			return tmp;
  ...
  public:
	...
	/**
	 * Constructs the container with `count` [default-inserted](https://en.cppreference.com/w/cpp/named_req/DefaultInsertable). No copies are made.
	 * Parameters:
	 *   - alloc: allocator to use for all memory allocations of this container
	 *   - count: the size of the container
	 *   - value: the value to initialize elements of the container with
	...
	/**
	 * Constructs the container with the contents of the range `[first, last)`. This overload participates in overload resolution only if `InputIt` satisfies [LegacyInputIterator](https://en.cppreference.com/w/cpp/named_req/InputIterator), to avoid ambiguity with the overload.
	 * Parameters:
	 *   - alloc: allocator to use for all memory allocations of this container
	...
	/**
	 * Copy constructor. Constructs the container with the copy of the contents of `other`. The allocator is obtained as if by calling `[std::allocator_traits](http://en.cppreference.com/w/cpp/memory/allocator_traits)<allocator_type>::select_on_container_copy_construction(other.get_allocator())`.
	 * Parameters:
	...
	/**
	 * Constructs the container with the copy of the contents of `other`, using `alloc` as the allocator. The template parameter `Allocator` is only deduced from the first argument while used in [class template argument deduction](https://en.cppreference.com/w/cpp/language/class_template_argument_deduction).
	 * Parameters:
	 *   - other: another container to be used as source to initialize the elements of the container with
	 *   - alloc: allocator to use for all memory allocations of this container
	 * Complexity: Linear in size of `other`
	 ...
	 * Notes:
	 * After container move construction, references, pointers, and iterators (other than the end iterator) to other remain valid, but refer to elements that are now in `*this`. The current standard makes this guarantee via the blanket statement in [[container.rev.reqmts]/17](http://eel.is/c++draft/container.rev.reqmts#17), and a more direct guarantee is under consideration via [LWG 2321](https://cplusplus.github.io/LWG/issue2321).
	 */
	xorlist(xorlist &&other) noexcept(std::is_nothrow_move_constructible<T>::value) { splice(end(), other); }

	/**
	 * Allocator-extended move constructor. Using alloc as the allocator for the new container, moving the contents from other; if `alloc != other.get_allocator()`, this results in an element-wise move. The template parameter `Allocator` is only deduced from the first argument while used in [class template argument deduction](https://en.cppreference.com/w/cpp/language/class_template_argument_deduction).
	 * Parameters:
	 *   - other: another container to be used as source to initialize the elements of the container with
	 *   - alloc: allocator to use for all memory allocations of this container
	...
	xorlist &operator=(xorlist &&other) noexcept(std::allocator_traits<Allocator>::is_always_equal::value) {
		_move_assign(other, integral_constant < bool,
	...
	/*
	 * Replaces the contents with copies of those in the range `[first, last)`. The behavior is undefined if either argument is an iterator into `*this`. This overload participates in overload resolution only if `InputIt` satisfies [LegacyInputIterator](https://en.cppreference.com/w/cpp/named_req/InputIterator).
	 * Parameters:
	 *   - first, last: the range to copy the elements from
	 * Complexity: Linear in distance between `first` and `last`
	 */
	...
	/*
	 * Returns a reference to the first element in the container. Calling `front` on an empty container causes [undefined behavior](https://en.cppreference.com/w/cpp/language/ub).
	...
	/*
	 * Returns a reference to the last element in the container. Calling `back` on an empty container causes [undefined behavior](https://en.cppreference.com/w/cpp/language/ub).
	 ...
	 * Return value: Reverse iterator to the first element.
	 * Complexity: Constant.
	 */
	 ...
	 * Return value: Reverse iterator to the element following the last element.
	 * Complexity: Constant.
	 */
	 ...
	 * Complexity: Constant.
	 * Notes: This value typically reflects the theoretical limit on the size of the container, at most `std::numeric_limits<difference_type>::max()`. At runtime, the size of the container may be limited to a value smaller than `max_size()` by the amount of RAM available.
	 */
	 ...
	 * This overload participates in overload resolution only if `InputIt` qualifies as [LegacyInputIterator](https://en.cppreference.com/w/cpp/named_req/InputIterator), to avoid ambiguity with the overload. The behavior is undefined if `first` and `last` are iterators into `*this`.
	 * Parameters:
	 *   - pos: iterator before which the content will be inserted. `pos` may be the `end()` iterator
	 *   - first, last: the range of elements to insert, can't be iterators into container for which insert is called Type requirements:
	...
	/*
	 * Inserts a new element into the container directly before `pos`. The element is constructed through [`std::allocator_traits::construct`](https://en.cppreference.com/w/cpp/memory/allocator_traits/construct), which uses placement-new to construct the element in-place at a location provided by the container. The arguments `args...` are forwarded to the constructor as `std::forward<Args>(args)...`. `args...` may directly or indirectly refer to a value in the container. No iterators or references are invalidated.
	 * Parameters:
	 ...
	 * Complexity: Constant.
	 * Exceptions: If an exception is thrown (which can be due to `Allocator::allocate()` or element copy/move constructor/assignment), this function has no effect ([strong exception guarantee](https://en.cppreference.com/w/cpp/language/exceptions#Exception_safety)).
	 */
	 ...
	 *   - value: the value of the element to prepend
	 * Complexity: Constant.
	 * Exceptions: If an exception is thrown, this function has no effect ([strong exception guarantee](https://en.cppreference.com/w/cpp/language/exceptions#Exception_safety)).
	 */
	 ...
	 *   - args: arguments to forward to the constructor of the element
	 * Type requirements
	 *   - `T` must meet the requirements of [EmplaceConstructible](https://en.cppreference.com/w/cpp/named_req/EmplaceConstructible).
	 * Return value: A reference to the inserted element.
	 * Complexity: Constant.
	 * Exceptions: If an exception is thrown, this function has no effect (strong exception guarantee).
	 */
	...
	/*
	 * Removes the first element of the container. If there are no elements in the container, the behavior is undefined. References and iterators to the erased element are invalidated.
	 * Complexity: Constant.
	 ...
	 * If the current size is less than `count`, additional [default-inserted](https://en.cppreference.com/w/cpp/named_req/DefaultInsertable) elements are appended.
	 * Parameters:
	 ...
	 * Notes: If value-initialization is undesirable, for example, if the elements are of non-class type and zeroing out is not needed, it can be avoided by providing a [custom `Allocator::construct`](http://stackoverflow.com/a/21028912/273767).
	 */
	 ...
	 * Otherwise, merges two sorted lists into one. The lists should be sorted into ascending order. No elements are copied. The container `other` becomes empty after the operation. No iterators or references become invalidated, except that the iterators of moved elements now refer into `*this`, not into `other`. Uses `operator<` to compare the elements.
	 ...
	 * Exceptions: If an exception is thrown, this function has no effect (strong exception guarantee), except if the exception comes from a comparison.
	 * Complexity:
	 * If `other refers to the same object as `*this`, no comparisons are performed.
	 * Otherwise, given $M$ and $N$ as `std::distance(begin(), end())` and `std::distance(other.begin(), other.end())`: at most $M + N - 1$ comparisons using `operator<`
	 */
	...
	/*
	 * The function does nothing if `other` refers to the same object as `*this`.
	 * Otherwise, merges two sorted lists into one. The lists should be sorted into ascending order. No elements are copied. The container `other` becomes empty after the operation. No iterators or references become invalidated, except that the iterators of moved elements now refer into `*this`, not into `other`. Uses  the given comparison function `comp` to compare the elements.
	 * This operation is stable: for equivalent elements in the two lists, the elements from `*this` shall always precede the elements from `other`, and the order of equivalent elements of `*this` and `other` does not change.
	 * If `get_allocator() != * other.get_allocator()`, the behavior is undefined.
	 * Parameters:
	 *   - other: another container to merge
	 *   - comp:
		   ...
		   While the signature does not need to have `const &`, the function must not modify the objects passed to it and must be able to accept all values of type (possibly `const`) `Type1` and `Type2` regardless of [value category]( the given comparison function comp) (thus, `Type1 &` is not allowed, nor is `Type1` unless for `Type1` a move is equivalent to a copy). The types `Type1` and `Type2` must be such that an object of type `list<T, Allocator>::const_iterator` can be dereferenced and then implicitly converted to both of them.
	 * Exceptions: If an exception is thrown, this function has no effect (strong exception guarantee), except if the exception comes from a comparison.
	 * Complexity:
	 * If `other refers to the same object as `*this`, no comparisons are performed.
	 * Otherwise, given $M$ and $N$ as `std::distance(begin(), end())` and `std::distance(other.begin(), other.end())`: at most $M + N - 1$ comparisons using `comp`
	 */
	 ...
	 *     The expression `p(v)` must be convertible to `bool` for every argument `v` of type (possibly `const`) `T`, regardless of [value category](https://en.cppreference.com/w/cpp/language/value_category), and must not modify `v`.
	 ...
	 * Notes:
	 * [`std::sort`](https://en.cppreference.com/w/cpp/algorithm/sort) requires random access iterators and so cannot be used with list. This function also differs from [`std::sort`](https://en.cppreference.com/w/cpp/algorithm/sort) in that it does not require the element type of the list to be swappable, preserves the values of all iterators, and performs a stable sort.
	 */
	void sort() { sort(std::less<value_type>()); }

	/*
	 * Sorts the elements in ascending order. The order of equal elements is preserved. Uses the given comparison function `comp` to compare the elements.
	 * If an exception is thrown, the order of elements in `*this` is unspecified.
	 * Parameters:
	 *   - comp:
	 *     comparison function object (i.e. an object that satisfies the requirements of [Compare](https://en.cppreference.com/w/cpp/named_req/Compare)) which returns ​`true` if the first argument is *less* than (i.e. is ordered *before*) the second.
	 *     The signature of the comparison function should be equivalent to the following: `bool cmp(const Type1 &a, const Type2 &b);`
	 *     While the signature does not need to have `const &`, the function must not modify the objects passed to it and must be able to accept all values of type (possibly `const`) `Type1` and `Type2` regardless of [value category](https://en.cppreference.com/w/cpp/language/value_category) (thus, `Type1 &` is not allowed, nor is `Type1` unless for `Type1` a move is equivalent to a copy). The types `Type1` and `Type2` must be such that an object of type `xorlist<T,Allocator>::const_iterator` can be dereferenced and then implicitly converted to both of them.
	 * Complexity: Approximately `N log N` comparisons, where `N` is the number of elements* in the list.
	 * Notes:
	 * [`std::sort`](https://en.cppreference.com/w/cpp/algorithm/sort) requires random access iterators and so cannot be used with list. This function also differs from [`std::sort`](https://en.cppreference.com/w/cpp/algorithm/sort) in that it does not require the element type of the list to be swappable, preserves the values of all iterators, and performs a stable sort.
	 */
 ...
 * Type requirements:
 *  - T must meet the requirements of [EqualityComparable](https://en.cppreference.com/w/cpp/named_req/EqualityComparable) in order to use overloads.
...
bool operator==(const xorlist<value_type, Alloc> &lhs, const xorlist<value_type, Alloc> &rhs) {
	return lhs.size() == rhs.size() && std::equal(lhs.begin(), lhs.end(), rhs.begin());
}

/*
 * Compares the contents of two lists.
 * Compares the contents of `lhs` and `rhs` lexicographically. The comparison is performed as if by calling [`std::lexicographical_compare_three_way`](https://en.cppreference.com/w/cpp/algorithm/lexicographical_compare_three_way) on two lists with a function object performing synthesized three-way comparison. The return type is same as the result type of synthesized three-way comparison.
 ...
 *   - otherwise, if comparing two `const E` lvalues by `operator<` is well-formed and the result type satisfies [boolean-testable](https://en.cppreference.com/w/cpp/concepts/boolean-testable), equivalent to
 *     ```cpp
 ...
 * Complexity: Constant.
 * Exceptions: [noexcept](https://en.cppreference.com/w/cpp/language/noexcept_spec) specification: `noexcept(noexcept(lhs.swap(rhs)))`
 * Notes:
 * Although the overloads of [`std::swap`](https://en.cppreference.com/w/cpp/algorithm/swap) for container adaptors are introduced in C++11, container adaptors can already be swapped by [`std::swap`](https://en.cppreference.com/w/cpp/algorithm/swap) in C++98. Such calls to [`std::swap`](https://en.cppreference.com/w/cpp/algorithm/swap) usually have linear time complexity, but better complexity may be provided.
 */
template <class value_type, class Alloc>
 ...
 * Complexity: Linear.
 * Notes:
 * Unlike [`xorlist::remove`](https://en.cppreference.com/w/cpp/container/list/remove), erase accepts heterogenous types and does not force a conversion to the container's value type before invoking the `==` operator.
 */
 ...
 *     The expression `pred(v)` must be convertible to `bool` for every argument `v` of type (possibly `const`) `T`, regardless of [value category](https://en.cppreference.com/w/cpp/language/value_category), and must not modify `v`.
 *     Thus, a parameter type of `T&` is not allowed, nor is `T` unless for `T` a move is equivalent to a copy.
 * Return value: The number of erased elements.
 * Complexity: Linear.
 * Notes:
 * Unlike `xorlist::remove`, erase accepts heterogenous types and does not force a conversion to the container's value type before invoking the `==` operator.
 */
...
/*
* This [deduction guide](https://en.cppreference.com/w/cpp/language/class_template_argument_deduction) is provided for list to allow deduction from an iterator range. This overload participates in overload resolution only if InputIt satisfies [LegacyInputIterator](https://en.cppreference.com/w/cpp/named_req/InputIterator) and Alloc satisfies [Allocator](https://en.cppreference.com/w/cpp/named_req/Allocator).
* Note:
* The extent to which the library determines that a type does not satisfy [LegacyInputIterator](https://en.cppreference.com/w/cpp/named_req/InputIterator) is unspecified, except that as a minimum integral types do not qualify as input iterators. Likewise, the extent to which it determines that a type does not satisfy [Allocator](https://en.cppreference.com/w/cpp/named_req/Allocator) is unspecified, except that as a minimum the member type `Alloc::value_type` must exist and the expression `std::declval<Alloc&>().allocate(std::size_t{})` must be well-formed when treated as an unevaluated operand.
*/
